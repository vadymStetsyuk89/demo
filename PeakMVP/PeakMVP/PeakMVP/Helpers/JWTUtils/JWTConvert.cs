...
namespace PeakMVP.Helpers.JWTUtils {
    ...
    public static class JWTConvert {
        ...
        public static string Decode(string token, byte[] key, bool verify = true) {
            ...
            var headerJson = Encoding.UTF8.GetString(decodedHeader, 0, decodedHeader.Length);
            var headerData = JsonConvert.DeserializeObject<Dictionary<string, object>>(headerJson);
            var payloadJson = Encoding.UTF8.GetString(decodedPayload, 0, decodedPayload.Length);
        ...
        public static object DecodeToObject(string token, string key, bool verify = true) {
            var payloadJson = JWTConvert.Decode(token, key, verify);
